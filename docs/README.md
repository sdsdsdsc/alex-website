...
- Community Map categories: the Phase 15B-3 implementation uses `assetType` as the preferred presentation source and the legacy `category` field only as a fallback. Five controlled, project-owned blue marker symbols and Layers-tab visibility controls affect Map display only; unknown values use a fallback symbol. No stored-data migration occurred, and Search and Filters remain unchanged. Status: committed on `codex/community-category-icons`, pushed to GitHub, and open in draft PR #55; push and pull-request verification passed; not yet approved or merged, not deployed, and production remains on `main` without this PR.
