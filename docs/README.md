...
- Historical audits, evidence reviews, geometry investigations, provider
  reconciliation, and misleading-risk reviews belong in `docs/audits/`.
- Batch, implementation, migration, and roadmap plans belong in `docs/plans/`.
- Non-production research drafts and evidence belong in `docs/research/`,
  including draft GeoJSON, KML/KMZ, coordinate captures, tracing notes,
  research exports, and preserved temporary analyses.
- Supporting screenshots, diagrams, prototype SVGs, comparison images, and
  documentation illustrations belong in `docs/assets/`.
- Only durable repository-wide documents, such as this index and
  `docs/project-status-checkpoint.md`, belong directly under `docs/`.
- Production source data and generated application outputs remain in their
  established application data paths. They must not be moved into
  `docs/research/` or `docs/assets/`.

Future Codex tasks must create files directly in the appropriate existing
folder, avoid new Phase documents at the root of `docs/`, use these categories
before proposing another one, update every internal reference when a file
moves, distinguish research drafts from production data, exclude temporary
workspace status from durable documents, and avoid duplicating a controlling
policy inside a historical plan.
...
- [docs/phase-15c-2-official-geometry-rendering.md](./phase-15c-2-official-geometry-rendering.md): Meaning-driven static line and area styles, feature-level Leaflet rendering, accessible non-Point activation, safe provenance-aware popups, official-category integration, atomic failure, and synthetic coverage for all validated geometry families. Status: PR #62 merged using a merge commit, resulting in `main` HEAD `7354de5cb349a68a3676643b059b59382edb4f31`. Post-merge verification run 323 and Pages build and deployment run 368 passed, and production is live. The official layer remains off by default; enabling it displays exactly five official Point markers. No real line, polygon, area, or other non-Point geometry is visible yet. Official category controls remain functional: disabling Ancient buildings leaves the two modern historic-site markers and puts the parent control into the expected mixed state, while restoring Ancient buildings returns all five official markers. Five community markers remain visible; Search, Filters, and community category controls remain unchanged; and no application-owned browser warnings or errors were recorded. No real-geometry implementation PR has started.
- [docs/policy/official-record-publication-policy.md](./policy/official-record-publication-policy.md): Controlling current policy for official-record publication outcomes, evidence thresholds, representation meanings, upgrades, identity/counting, parent/component relationships, provenance, uncertainty, sensitivity, misleading risk, and the smallest future multi-representation schema extension. Status: policy authority only; it does not approve any record, implementation batch, or publication.
- [docs/audits/phase-15c-3-first-real-official-geometry.md](./audits/phase-15c-3-first-real-official-geometry.md): Historical first-candidate audit using the original strict evidence gate. It withheld every candidate from publication, with Xieli recorded as the strongest but still unresolved option. Status: preserved for audit history and superseded by the Phase 15C-4 re-audit and standalone official-record publication policy; it is not implementation approval.
- [docs/audits/phase-15c-4-xinyu-mixed-geometry-reaudit.md](./audits/phase-15c-4-xinyu-mixed-geometry-reaudit.md): Historical mixed-geometry re-audit that introduced project-reference geometry and recommended a single Xieli generalized reference area under its then-current classification scheme. Status: preserved for audit history; its classification scheme and batch recommendation are superseded by Phase 15C-6, Phase 15C-7, and the standalone policy.
- [docs/audits/phase-15c-5-xiabu-geometry-pilot.md](./audits/phase-15c-5-xiabu-geometry-pilot.md): Authoritative detailed Xiabu evidence record covering the Gaode, Baidu, Google, plaque-identity, coordinate-reconciliation, uncertainty, and Point-semantics review. Status: evidence review only. Xiabu is a proposed future Point candidate and must not be implemented or published without explicit approval.
- [docs/plans/phase-15c-6-official-record-publication-policy-and-batch-plan.md](./plans/phase-15c-6-official-record-publication-policy-and-batch-plan.md): Historical policy-development and Xinyu batch-planning record. It preserves the complete audit table, Xiabu worked example, proposed batch, reserve and withheld candidates, evidence gaps, and implementation sequence. Status: historical plan; its reusable policy has moved to the standalone controlling policy, and its Xieli shape proposal is superseded by Phase 15C-7.
- [docs/audits/phase-15c-7-xieli-misleading-risk-review.md](./audits/phase-15c-7-xieli-misleading-risk-review.md): Authoritative Xieli misleading-risk review comparing the theoretical 60 × 60 m square, a 500 m uncertainty area, a generalized Point, and withholding at regional, neighbourhood, and site zooms. Status: rejects the square and broader uncertainty area as visually misleading, recommends a generalized reference Point only as a future separately approved alternative, and retains withholding as the fallback. Xieli remains unpublished.
