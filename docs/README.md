...
- Official Map categories: the bounded Phase 15B-5 implementation on `codex/official-category-filters` adds presentation-only orange marker glyphs and Layers-tab visibility controls derived only from validated published features. Exact non-empty `officialCategoryZh` values map to controlled English project-presentation labels; official source values remain unchanged, and unrecognized future values use a static fallback. Source records without approved public locations do not appear in Map category controls or counts. Filtering changes Map visibility only and does not change public data, the GeoJSON schema, URLs, map bounds, community behavior, Search, or Filters. Status: implementation committed and pushed to `codex/official-category-filters`; draft PR #58 is open, and push and pull-request verification passed. PR #58 remains unapproved, unmerged, and undeployed; production remains unchanged on `main`.
