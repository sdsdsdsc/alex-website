...
- Map Layers tab: local Phase 15B-2 UI implementation adds an authoritative, accessible fourth tab to the existing Heritage Explorer for the community and provincial overlays while retaining the separate Leaflet basemap chooser and full-width Map column. Status: implemented and verified locally on `codex/map-layers-sidebar`; not committed, pushed, reviewed, or deployed.
