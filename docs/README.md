...
- [docs/phase-15c-1-official-geometry-schema-foundation.md](./phase-15c-1-official-geometry-schema-foundation.md): Backward-compatible geometry structure, meaning, provenance, precision, uncertainty, compatibility, atomic validation, accessible labels, and the explicit Point-only production-renderer boundary for future official heritage geometry. Status: implementation committed and pushed to `codex/official-geometry-schema-foundation`; draft PR #60 is open, latest GitHub push verification run 311 and latest pull-request verification run 312 passed, and the PR remains unapproved, unmerged, and undeployed. Production remains unchanged on `main`: no non-Point geometry is published, the current five Point features remain byte-for-byte unchanged, production rendering remains Point-only, and PR 5B rendering work has not started.
