...
The project is past architecture setup and safe-audit work. Its public place, search, map, article, nomination, review, promotion, export, and public-user auth/account pathways are implemented, though release assurance remains an ongoing responsibility. Phase 14A–14F are complete for the approved ten-record provincial protected heritage pilot. The Phase 15B-1 official marker foundation, Phase 15B-2 Heritage Explorer Layers tab, Phase 15B-3 community category icons, bounded Phase 15B-4 Xinyu expansion, Phase 15B-5 official category controls, and Phase 15C-1 / PR 5A geometry schema foundation are merged and production verified. PR #60 merged using a merge commit, resulting in `main` HEAD `ab3128b958a1f3fb155f101a89a24ce2226aa739`; post-merge verification run `30355339498` and Pages build and deployment run `30355337278` passed, and production is live. The official layer remains off by default. Enabling it displays exactly five official Point markers. Official category controls remain functional: disabling Ancient buildings leaves the two modern historic-site markers and produces the expected mixed parent state, while restoring Ancient buildings returns all five. Five community markers remain visible; Search, Filters, and community category controls remain unchanged. No line, polygon, or area geometry is visible, and no application-owned console warnings or errors were recorded. The practical position is: **Phase 15C-1 / PR 5A is complete in production, the Point-only renderer boundary remains in force, and PR 5B rendering work has not started.**
