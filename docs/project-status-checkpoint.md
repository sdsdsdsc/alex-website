...
| Phase 15B-5 | Official category glyphs and Map visibility controls | Implemented and verified locally; unstaged and uncommitted | Uses exact non-empty `officialCategoryZh` values to assign controlled English project-presentation labels and static orange glyphs. Controls are created only from validated published features, so source records without approved public locations do not appear in category controls or counts. Filtering is page-session Map visibility only; it preserves lazy loading, cached validated features, URLs, map bounds, community categories, Search, Filters, source values, public-location decisions, and the GeoJSON schema. | The work remains local on `codex/official-category-filters`; it is not committed, pushed, reviewed, deployed, or merged. No new official records, coordinates, source categories, or publication decisions were added. | Preserve the presentation-only boundary and wait for explicit approval before staging or committing. |
