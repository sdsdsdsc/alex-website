...
| Phase 15C-16 | Generalized reference Point eligibility audit | Research and documentation complete; implementation not approved | Distinguishes coordinate uncertainty from deliberate spatial generalization, applies a ten-gate eligibility test to all 55 unpublished public identities, and shortlists only Xieli Site and Qipanshan Site. | Neither candidate is presently eligible. Both are potentially suitable only after candidate-specific evidence, a reproducible generalized-location method, separated uncertainty components, risk review, accountable approval, and publication approval. The future implementation batch is empty. | Preserve the zero-eligible result. Do not publish a generalized reference Point without satisfying every Phase 15C-16 gate. |
| Official Heritage spatial policy and roadmap | Current controlling documentation | Defines five simplified public types, evidence-led natural forms, one active public representation per identity, authority-versus-project representation status, rejection gates, the provider-located project-reviewed Point evidence method, the generalized-reference-Point eligibility distinction, and future sidebar/filter logic. Records completed PRs #67–#73 and the still-ordered but unnumbered lifecycle, interface, first-shape, context, and later-batch work. | Phase 15C-1 / PR #60 validation and Phase 15C-2 / PR #62 mixed-geometry rendering are production verified; no real line/area or one-active-representation lifecycle is published. Live production contains seven ordinary Points and no generalized reference Point. | Keep the Phase 15C-16 future batch empty. Do not assign future implementation PR numbers in advance. |
...
displays exactly seven ordinary official Point markers in live production: one
national and six provincial. No generalized reference Point, real line,
polygon, area, or other non-Point geometry is visible. Community Heritage
remains Point-based and unchanged.
...
Phase 15C-16 then applies the clarified generalized-reference-Point eligibility
test to all 55 unpublished public identities. It shortlists Xieli and Qipanshan
but finds neither presently eligible, leaving the future batch empty. The
...
meeting-site component, Xieli, and Qipanshan remain unpublished. The practical
position is: **Phase 15C-2 / PR 5B remains the renderer baseline; PR #69
establishes the seven-Point data baseline; the merged audits preserve the
evidence gates; and Phase 15C-16 approves no generalized-reference-Point
implementation.**
...
7. extend the existing geometry schema and validators for stable identity,
   active representation, supersession, and representation history;
8. adapt the existing mixed-geometry renderer and Official Heritage controls
   for the five-type model, active-representation selection, provenance, and
   accessibility;
9. publish and verify the first separately approved production shape; and
10. consider context layers and later evidence-supported batches separately.
...
### Phase 15C-16 Generalized reference Point eligibility audit — documentation only

Preserve the verified seven ordinary production Points and assess only whether
any of the 55 unpublished public identities can honestly use the existing
Generalized reference Point representation. The controlling audit shortlists
Xieli Site and Qipanshan Site but approves neither: each still requires
candidate-specific evidence, a reproducible generalized-location method,
separate source/datum/generalization uncertainty, risk review, accountable
review, and publication approval. The ordered future batch is empty. Do not
change data, runtime, schema implementation, rendering, Community Heritage,
Firebase, or production in this documentation-only phase.
