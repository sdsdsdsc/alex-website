...
| Phase 15C-26 / PR #86 | M31 authority-shape evidence acquisition | Documentation research complete | Recovers the original Xinyu City government announcement plus its government-hosted `554 × 313` orthophoto annotation and `554 × 371` survey drawing. Confirms the low resolution is upstream, not merely a media-mirror resize. Finds no raw CAD/GIS, coordinate table, full projected CRS, reproducible WGS84 path, clear geometry-reuse permission or exact-grave disclosure approval. | M31 remains Outcome B. No provisional geometry is constructed; production remains 19 records, nine Points, ten exclusions and zero lines/areas. No other identity or infrastructure is changed. | Obtain original/raw coordinate-bearing survey material, complete CRS, reuse authorization and grave-disclosure approval before reconsidering Outcome A. |
...
and practical accessibility findings. An owner-requested re-check corrects the
initial overstatement about Official Heritage: the Layers tab is visible and
the Official Heritage controls are clear once Layers is open; the remaining
gaps concern site-wide framing, plain-language distinction and mobile distance
between controls and map. The audit now selects exactly one proposed next
implementation: repair the confirmed mobile overflow in the shared public
account layout. It changes no product behavior, production data, Firebase
configuration or rules, Community Heritage model, Official Heritage
decision/geometry artifact, or deployment state. PR #88 is not started and
...
### Phase 15C-26 M31 authority-shape evidence acquisition — PR #86

The original Xinyu City government notice and both official M31 inline images
are recovered, but the official survey drawing is itself only `554 × 371` and
has no usable grid/control values. No raw spatial file, full projected CRS,
WGS84 transformation, reusable-geometry permission or exact-grave disclosure
approval is recovered. Preserve M31 as Outcome B; do not construct or publish a
shape. Production stays at 19 records, nine Points, ten exclusions, zero lines
and zero areas. No other candidate is reopened and no infrastructure change is
authorized.
