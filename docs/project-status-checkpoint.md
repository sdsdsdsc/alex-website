...
| Phase 15B-2 | Heritage Explorer Layers-tab UI architecture | Committed and pushed; draft PR #53 open | Replaces the temporary overlay toggles with an accessible fourth tab in the existing Heritage Explorer for the community and provincial layers. The Map retains the original two-column page width, and the separate Leaflet control retains basemap choices only; community remains on by default and official heritage remains off and lazy-loaded. | Committed on `codex/map-layers-sidebar`, pushed to GitHub, and open in draft PR #53; not yet approved, merged, or deployed. Production remains on `main` without this PR. No new data, filters, geometry, or official-list product is included. | Complete review of draft PR #53 before any approval or merge. |
...
The project is past architecture setup and safe-audit work. Its public place, search, map, article, nomination, review, promotion, export, and public-user auth/account pathways are implemented, though release assurance remains an ongoing responsibility. Phase 14A–14F are complete for the approved ten-record provincial protected heritage pilot, and Phase 15B-1 is merged and production verified with one separately reviewed 新余孔庙 reference marker. A bounded Phase 15B-2 Heritage Explorer Layers tab is committed on `codex/map-layers-sidebar`, pushed to GitHub, and open in draft PR #53, but is not yet approved, merged, or deployed; production remains on `main` without this PR. The practical position is: **the official marker foundation is live; the Layers tab is awaiting review in draft PR #53; the public non-map list and any expansion remain separately approved future work.**
