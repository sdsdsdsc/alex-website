...
| Phase 15B-5 | Official category glyphs and Map visibility controls | Committed and pushed; draft PR #58 open with push and pull-request verification passed | Uses exact non-empty `officialCategoryZh` values to assign controlled English project-presentation labels and static orange glyphs. Controls are created only from validated published features, so source records without approved public locations do not appear in category controls or counts. Filtering is page-session Map visibility only; it preserves lazy loading, cached validated features, URLs, map bounds, community categories, Search, Filters, source values, public-location decisions, and the GeoJSON schema. | The implementation is pushed to `codex/official-category-filters`. PR #58 remains unapproved, unmerged, and undeployed, and production remains unchanged on `main`. No new official records, coordinates, source categories, or publication decisions were added. | Preserve the presentation-only boundary; do not merge or deploy without explicit approval. |
...
The project is past architecture setup and safe-audit work. Its public place, search, map, article, nomination, review, promotion, export, and public-user auth/account pathways are implemented, though release assurance remains an ongoing responsibility. Phase 14A–14F are complete for the approved ten-record provincial protected heritage pilot. The Phase 15B-1 official marker foundation, Phase 15B-2 Heritage Explorer Layers tab, Phase 15B-3 community category icons, and bounded Phase 15B-4 Xinyu expansion are merged and production verified. PR #56 was merged using a merge commit, resulting in `main` HEAD `ecdf29198cdd430272cbfce2b9ababb43f99f802`; post-merge verification and the Pages build and deployment passed. Production is live with five approved official markers, and the official layer remains off by default. Phase 15B-5 official category glyphs and Map-only visibility controls are committed and pushed to `codex/official-category-filters`; they are derived only from validated published features and do not change source values, public data, schema, map bounds, community behavior, Search, or Filters. Draft PR #58 is open, and push and pull-request verification passed. The practical position is: **Phase 15B-4 remains complete in production; PR #58 remains unapproved, unmerged, and undeployed, and production remains unchanged on `main`.**
