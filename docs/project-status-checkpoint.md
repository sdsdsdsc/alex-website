...
| Phase 15B-2 | Heritage Explorer Layers-tab UI architecture | Implemented and verified locally; not committed | Replaces the temporary overlay toggles with an accessible fourth tab in the existing Heritage Explorer for the community and provincial layers. The Map retains the original two-column page width, and the separate Leaflet control retains basemap choices only; community remains on by default and official heritage remains off and lazy-loaded. | The local branch is not staged, committed, pushed, reviewed, or deployed. No new data, filters, geometry, or official-list product is included. | Review the bounded local diff before approving any commit or pull request. |
...
The project is past architecture setup and safe-audit work. Its public place, search, map, article, nomination, review, promotion, export, and public-user auth/account pathways are implemented, though release assurance remains an ongoing responsibility. Phase 14A–14F are complete for the approved ten-record provincial protected heritage pilot, and Phase 15B-1 is merged and production verified with one separately reviewed 新余孔庙 reference marker. A bounded Phase 15B-2 Heritage Explorer Layers tab is implemented and verified locally but is not committed or deployed. The practical position is: **the official marker foundation is live; the Layers tab is local review work only; the public non-map list and any expansion remain separately approved future work.**
